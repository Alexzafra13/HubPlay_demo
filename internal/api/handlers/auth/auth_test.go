...
// ─── Fake library service (for auth register flow) ─────────────────────────
...
// Satisfy the wider LibraryService interface methods the auth handler
// might call (CreatePersonalIPTV, ListAccessByUser are unused but the
// authLibraryOps interface only demands GetByID + ReplaceAccess, so
...
func TestUserHandler_Me(t *testing.T) {
	...
	handler := users.NewUserHandler(userSvc, nil, nil, testLogger())
...
func TestUserHandler_Me_Unauthenticated(t *testing.T) {
	t.Parallel()
	handler := users.NewUserHandler(&mockUserService{}, nil, nil, testLogger())
