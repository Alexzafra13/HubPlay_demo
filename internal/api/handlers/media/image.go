...
type ImageHandler struct {
	...
	refresher   handlers.ImageRefreshService
	imageDir    string
...
func (h *ImageHandler) auditEmit() handlers.AuditEmitter {
	if h.audit != nil {
		return h.audit
	}
...
func (h *ImageHandler) List(w http.ResponseWriter, r *http.Request) {
	itemID := handlers.RequireParam(w, r, "id")
	if itemID == "" {
		return
	}
...
func (h *ImageHandler) Available(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "failed to get external IDs")
		return
	}
	...
	if err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "failed to fetch images from providers")
		return
	}
...
func (h *ImageHandler) Select(w http.ResponseWriter, r *http.Request) {
	...
	imgType := handlers.RequireParam(w, r, "type")
	if imgType == "" {
		return
	}

	if !imaging.IsSafePathSegment(itemID) {
		handlers.RespondError(w, r, http.StatusBadRequest, "VALIDATION_ERROR", "invalid item id")
		return
	}
	if !imaging.IsValidKind(imgType) {
		handlers.RespondError(w, r, http.StatusBadRequest, "VALIDATION_ERROR", "invalid image type")
		return
	}
	...
	if body.URL == "" {
		handlers.RespondError(w, r, http.StatusBadRequest, "VALIDATION_ERROR", "url is required")
		return
	}
	...
	if err != nil {
		...
		handlers.RespondError(w, r, http.StatusBadGateway, "DOWNLOAD_FAILED", "failed to download image")
		return
	}
	...
	if err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "failed to save image")
		return
	}
	h.auditEmit().LogArtworkChanged(r.Context(), r, "item_id", itemID, imgType)

	handlers.RespondData(w, http.StatusOK, imageResponse(img))
}
...
func (h *ImageHandler) SetLocked(w http.ResponseWriter, r *http.Request) {
	...
	imageID := handlers.RequireParam(w, r, "imageId")
	if imageID == "" {
		return
	}
	...
	if img.ItemID != itemID {
		handlers.RespondAppError(w, r.Context(), domain.NewNotFound("image"))
		return
	}
	...
	if err := h.images.SetLocked(r.Context(), imageID, body.Locked); err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "failed to set lock")
		return
	}
...
func (h *ImageHandler) SetPrimary(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.images.SetPrimary(r.Context(), itemID, img.Type, imageID); err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "failed to set primary image")
		return
	}
...
func (h *ImageHandler) Delete(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.images.DeleteByID(r.Context(), imageID); err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "failed to delete image")
		return
	}
...
func (h *ImageHandler) RefreshLibraryImages(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "failed to refresh images")
		return
	}

	handlers.RespondData(w, http.StatusOK, map[string]any{"updated": updated})
}
...
func (h *ImageHandler) ServeFile(w http.ResponseWriter, r *http.Request) {
	imageID := handlers.RequireParam(w, r, "id")
	if imageID == "" {
		return
	}
...
func (h *ImageHandler) ServeImageByID(w http.ResponseWriter, r *http.Request, imageID string) {
	...
	if localPath == "" {
		...
		handlers.RespondError(w, r, http.StatusNotFound, "NOT_FOUND", "image file not found")
		return
	}
