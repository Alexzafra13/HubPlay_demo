...
func TestScanLibrary_ShowsBackfillsLegacyEpisodeRows(t *testing.T) {
	// Reported by user: tras el fix de la jerarquía, re-escaneé y los
	// episodios siguen apareciendo en /movies (well, no, en sus
	// queries) sin parent y /series sigue vacío. Reproducción exacta:
	//
	// 1. Lib escaneada con código pre-hierarchy → filas episode con
	//    ParentID = "".
	// 2. Re-scan con código nuevo: `processFile` ve fingerprint match
	//    y short-circuit'ea a `enrichIfMissing`, nunca llega al
	//    createItem nuevo. Los episodios huérfanos siguen sin parent.
	//
	// Fix: el branch del fingerprint-match detecta `episode + parent
	// vacío + lib.shows` y dispara el backfill.
	s, itemRepo, _ := newTestScanner(t)

	root := t.TempDir()
	full := filepath.Join(root, "Breaking Bad", "Season 01", "Breaking.Bad.S01E01.Pilot.mkv")
	if err := os.MkdirAll(filepath.Dir(full), 0o755); err != nil {
		t.Fatal(err)
	}
	createFile(t, full, "x")
	...
	// Plant a legacy row directly: type=episode, no parent, no S/E
	// numbers — exactly what the pre-hierarchy scanner would have
	// emitted. Same fingerprint as the file on disk so the next scan
	...
	legacy := &db.Item{
		...
		SortTitle:   "breaking.bad.s01e01.pilot",
		Path:        full,
		Fingerprint: fp,
		AddedAt:     now,
		UpdatedAt:   now,
	...
	if err := itemRepo.Create(context.Background(), legacy); err != nil {
		t.Fatal(err)
	}

	// Re-scan. The fingerprint matches → short-circuit branch fires.
	// The backfill must run and attach the legacy row to a fresh
	// series + season pair.
	if _, err := s.ScanLibrary(context.Background(), lib); err != nil {
		t.Fatal(err)
	}

	updated, err := itemRepo.GetByID(context.Background(), "legacy-ep-1")
	if err != nil {
		t.Fatal(err)
	}
	if updated.ParentID == "" {
		t.Fatal("legacy episode still has empty ParentID after backfill")
	}
	if updated.SeasonNumber == nil || *updated.SeasonNumber != 1 {
		t.Errorf("season number not set: %v", updated.SeasonNumber)
	}
	if updated.EpisodeNumber == nil || *updated.EpisodeNumber != 1 {
		t.Errorf("episode number not set: %v", updated.EpisodeNumber)
	}

	// And /series should now find rows of type=series for this lib.
	series, _, _ := itemRepo.List(context.Background(), db.ItemFilter{
		LibraryID: lib.ID, Type: "series", Limit: 10,
	})
	if len(series) != 1 || series[0].Title != "Breaking Bad" {
		t.Errorf("expected one Breaking Bad series row, got %v", series)
	}
}
