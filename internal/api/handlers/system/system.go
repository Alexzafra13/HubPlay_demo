...
func (h *SystemHandler) StreamActivity(w http.ResponseWriter, r *http.Request) {
	...
	if h.activity == nil {
		handlers.RespondError(w, r, http.StatusServiceUnavailable, "DB_UNAVAILABLE", "stream activity unavailable")
		return
	}
	buckets, err := h.activity.DailyWatchActivity(r.Context(), cutoff)
	if err != nil {
		h.logger.Error("stream activity query", "error", err)
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "stream activity unavailable")
		return
	}
...
func (h *SystemHandler) TopItems(w http.ResponseWriter, r *http.Request) {
	...
	if h.activity == nil {
		handlers.RespondError(w, r, http.StatusServiceUnavailable, "DB_UNAVAILABLE", "top items unavailable")
		return
	}
	rows, err := h.activity.TopItems(r.Context(), cutoff, limit)
	if err != nil {
		h.logger.Error("top items query", "error", err)
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "top items unavailable")
		return
	}
