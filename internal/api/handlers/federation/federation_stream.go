...
type FederationStreamHandler struct {
	...
	streams      handlers.StreamManagerService
	items        handlers.ItemRepository
	mediaStreams handlers.MediaStreamRepository
	logger       *slog.Logger
}
...
func (h *FederationStreamHandler) StartSession(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "share lookup failed")
		return
	}
...
func (h *FederationStreamHandler) MasterPlaylist(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := h.items.GetByID(r.Context(), sess.ItemID); err != nil {
		handlers.RespondError(w, r, http.StatusNotFound, "SESSION_NOT_FOUND", "session no longer valid")
		return
	}
...
func (h *FederationStreamHandler) QualityPlaylist(w http.ResponseWriter, r *http.Request) {
	...
	if ms.Decision.Method == stream.MethodDirectPlay {
		...
		handlers.RespondError(w, r, http.StatusConflict, "WRONG_METHOD", "session decided direct_play; use the direct path")
		return
	}

	manifestPath := ms.ManifestPath()
	if err := waitForFile(manifestPath, 10*time.Second); err != nil {
		handlers.RespondError(w, r, http.StatusServiceUnavailable, "MANIFEST_NOT_READY", "manifest still being generated")
		return
	}
	w.Header().Set("Content-Type", "application/vnd.apple.mpegurl")
	w.Header().Set("Cache-Control", handlers.CacheControlNoCache)
...
func (h *FederationStreamHandler) SubtitleTrack(w http.ResponseWriter, r *http.Request) {
	...
	trackIndex, err := strconv.Atoi(trackRaw)
	if err != nil {
		handlers.RespondError(w, r, http.StatusBadRequest, "INVALID_REQUEST", "trackIndex must be an integer")
		return
	}
...
func (h *FederationStreamHandler) lookupPeerSession(w http.ResponseWriter, r *http.Request, peerID string) *federation.PeerStreamSession {
	sid := handlers.RequireParam(w, r, "sessionId")
	if sid == "" {
		return nil
	}
	s := h.mgr.LookupPeerStreamSession(sid)
	if s == nil || s.PeerID != peerID {
		handlers.RespondError(w, r, http.StatusNotFound, "SESSION_NOT_FOUND", "session not found")
		return nil
	}
	return s
}
