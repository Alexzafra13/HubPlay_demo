...
func (h *AuditLogHandler) Query(w http.ResponseWriter, r *http.Request) {
	...
	if from := q.Get("from"); from != "" {
		if t, err := time.Parse(time.RFC3339, from); err == nil {
			...
			handlers.RespondError(w, r, http.StatusBadRequest, "BAD_FROM",
	...
	if to := q.Get("to"); to != "" {
		if t, err := time.Parse(time.RFC3339, to); err == nil {
			...
			handlers.RespondError(w, r, http.StatusBadRequest, "BAD_TO",
				"to must be RFC3339")
			return
		}
	}
	...
	if err != nil {
		handlers.RespondError(w, r, http.StatusInternalServerError, "QUERY_FAILED", err.Error())
		return
	}
...
func (h *AuditLogHandler) EventTypes(w http.ResponseWriter, r *http.Request) {
	...
	handlers.RespondData(w, http.StatusOK, types)
}
