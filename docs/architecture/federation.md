...
| **5 — Remote streaming + watch state** | **Slice 1 (backend) ✅ shipped 2026-05-04** (`bbafd56`): `POST /api/v1/peer/stream/{itemId}/session` honouring client capabilities, HLS proxy through origin, in-memory peer-stream session registry. **Slice 2 (frontend Plex-style + per-peer caps + `federation_progress`)** pending — see project-status.md "Sesión 2026-05-04 → Slice 2" for the detailed plan. | 2-3 days slice 1 + 3-5 days slice 2 | Play a peer's movie, transcode-only-if-needed, watch state on peer's side never mixes with your local users. |
