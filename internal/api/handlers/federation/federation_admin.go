...
func (h *FederationAdminHandler) GetServerIdentity(w http.ResponseWriter, r *http.Request) {
	...
	handlers.RespondData(w, http.StatusOK, infoToWire(info))
}
...
func (h *FederationAdminHandler) UpdateServerIdentity(w http.ResponseWriter, r *http.Request) {
	...
	if name == "" {
		handlers.RespondError(w, r, http.StatusBadRequest, "BAD_REQUEST", "name required")
		return
	}
	if len(name) > 80 {
		handlers.RespondError(w, r, http.StatusBadRequest, "BAD_REQUEST", "name too long (max 80 chars)")
		return
	}
	color := strings.TrimSpace(body.AvatarColor)
	if color != "" && !isValidHexColor(color) {
		handlers.RespondError(w, r, http.StatusBadRequest, "BAD_REQUEST", "avatar_color must be a hex like #rrggbb")
		return
	}
	if err := h.mgr.UpdateIdentityProfile(r.Context(), name, color); err != nil {
		h.logger.Error("update server identity", "error", err)
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL", "failed to update server identity")
		return
	}
	info := h.mgr.PublicServerInfo()
	if info.AdvertisedURL == "" {
...
func (h *FederationAdminHandler) UploadServerAvatar(w http.ResponseWriter, r *http.Request) {
	...
	if err := r.ParseMultipartForm(federation.IdentityAvatarMaxBytes + 1024); err != nil {
		handlers.RespondError(w, r, http.StatusRequestEntityTooLarge, "PAYLOAD_TOO_LARGE",
			"avatar exceeds the maximum allowed size")
		return
	}
	file, header, err := r.FormFile("avatar")
	if err != nil {
		handlers.RespondError(w, r, http.StatusBadRequest, "BAD_REQUEST",
	...
	if err != nil {
		handlers.RespondError(w, r, http.StatusBadRequest, "BAD_REQUEST", "could not read uploaded file")
		return
	}
...
func (h *FederationAdminHandler) GenerateInvite(w http.ResponseWriter, r *http.Request) {
	...
	if claims == nil {
		handlers.RespondError(w, r, http.StatusUnauthorized, "AUTH_REQUIRED", "unauthenticated")
		return
	}
	inv, err := h.mgr.GenerateInvite(r.Context(), claims.UserID)
	if err != nil {
		h.logger.Error("federation: generate invite", "error", err)
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "failed to generate invite")
		return
	}
	handlers.RespondData(w, http.StatusCreated, inviteWire{
		ID:        inv.ID,
		Code:      inv.Code,
...
func (h *FederationAdminHandler) ListActiveInvites(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "failed to list invites")
		return
	}
...
func (h *FederationAdminHandler) ProbePeer(w http.ResponseWriter, r *http.Request) {
	...
	if req.BaseURL == "" {
		handlers.RespondError(w, r, http.StatusBadRequest, "FEDERATION_BASE_URL_REQUIRED", "base_url required")
		return
	}
	...
	if err != nil {
		...
		handlers.RespondError(w, r, http.StatusBadGateway, "PEER_PROBE_FAILED",
...
func (h *FederationAdminHandler) AcceptInvite(w http.ResponseWriter, r *http.Request) {
	...
	if req.BaseURL == "" || req.Code == "" {
		handlers.RespondError(w, r, http.StatusBadRequest, "FEDERATION_REQUIRED_FIELDS_MISSING", "base_url and code required")
		return
	}
	...
	if err != nil {
		...
		handlers.RespondError(w, r, status, code, msg)
		return
	}
	handlers.RespondData(w, http.StatusCreated, peerToWire(peer))
}
...
func (h *FederationAdminHandler) ListPeers(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "failed to list peers")
		return
	}
...
func (h *FederationAdminHandler) GetPeer(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "failed to fetch peer")
		return
	}
	handlers.RespondData(w, http.StatusOK, peerToWire(p))
}
...
func (h *FederationAdminHandler) ListShares(w http.ResponseWriter, r *http.Request) {
	peerID := handlers.RequireParam(w, r, "id")
	if peerID == "" {
		return
	}
	...
	if err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "failed to list shares")
		return
	}
...
func (h *FederationAdminHandler) CreateShare(w http.ResponseWriter, r *http.Request) {
	...
	if req.LibraryID == "" {
		handlers.RespondError(w, r, http.StatusBadRequest, "FEDERATION_LIBRARY_REQUIRED", "library_id required")
		return
	}
	...
	handlers.RespondData(w, http.StatusCreated, shareToWire(share))
}
...
func (h *FederationAdminHandler) DeleteShare(w http.ResponseWriter, r *http.Request) {
	...
	shareID := handlers.RequireParam(w, r, "shareID")
	if shareID == "" {
		return
	}
	if err := h.mgr.UnshareLibrary(r.Context(), peerID, shareID); err != nil {
		h.logger.Error("federation: unshare library", "error", err)
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "failed to unshare")
		return
	}
	w.WriteHeader(http.StatusNoContent)
}
...
func (h *FederationAdminHandler) GetFederationSettings(w http.ResponseWriter, r *http.Request) {
	handlers.RespondData(w, http.StatusOK, federationSettingsWire{
...
func (h *FederationAdminHandler) UpdateFederationSettings(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.mgr.SetAcceptingPairingRequests(r.Context(), body.AcceptPairingRequests); err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL", "failed to update settings")
		return
	}
	handlers.RespondData(w, http.StatusOK, federationSettingsWire{
		AcceptPairingRequests: h.mgr.AcceptingPairingRequests(r.Context()),
...
func (h *FederationAdminHandler) SendPairingRequest(w http.ResponseWriter, r *http.Request) {
	...
	handlers.RespondData(w, http.StatusCreated, pendingRequestToWire(pending))
}
...
func (h *FederationAdminHandler) ListPairingRequests(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "failed to list pairing requests")
		return
	}
...
func (h *FederationAdminHandler) AcceptPairingRequest(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if errors.Is(err, domain.ErrNotFound) {
			handlers.RespondError(w, r, http.StatusNotFound, "REQUEST_NOT_FOUND", "request not found or expired")
			return
		}
		h.logger.Warn("federation: accept pairing request", "request_id", id, "error", err)
		handlers.RespondError(w, r, http.StatusBadRequest, "ACCEPT_FAILED", "could not accept pairing request")
		return
	}
	handlers.RespondData(w, http.StatusOK, peerToWire(peer))
}
...
func (h *FederationAdminHandler) DeclinePairingRequest(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.mgr.DeclinePairingRequest(r.Context(), id, claims.UserID); err != nil {
		...
		handlers.RespondError(w, r, http.StatusBadRequest, "DECLINE_FAILED", "could not decline pairing request")
		return
	}
	w.WriteHeader(http.StatusNoContent)
}
...
func (h *FederationAdminHandler) CancelPairingRequest(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.mgr.CancelPairingRequest(r.Context(), id, claims.UserID); err != nil {
		if errors.Is(err, domain.ErrNotFound) {
			handlers.RespondError(w, r, http.StatusNotFound, "REQUEST_NOT_FOUND", "request not found")
			return
		}
		h.logger.Warn("federation: cancel pairing request", "request_id", id, "error", err)
		handlers.RespondError(w, r, http.StatusBadRequest, "CANCEL_FAILED", "could not cancel pairing request")
		return
	}
	w.WriteHeader(http.StatusNoContent)
}
...
func (h *FederationAdminHandler) RefreshPeer(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		handlers.RespondError(w, r, http.StatusBadGateway, "PEER_REFRESH_FAILED",
...
func (h *FederationAdminHandler) RevokePeer(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.mgr.RevokePeer(r.Context(), id); err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "failed to revoke peer")
		return
	}
	w.WriteHeader(http.StatusNoContent)
}
