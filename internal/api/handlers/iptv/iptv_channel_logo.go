...
func (h *iptvLogoHandler) SetChannelLogo(w http.ResponseWriter, r *http.Request) {
	...
	if req.LogoURL == "" {
		handlers.RespondError(w, r, http.StatusBadRequest, "MISSING_URL", "logo_url required (use DELETE to clear)")
		return
	}
	if !isHTTPURL(req.LogoURL) {
		handlers.RespondError(w, r, http.StatusBadRequest, "INVALID_URL", "logo_url must be http(s)")
		return
	}
...
func (h *iptvLogoHandler) UploadChannelLogo(w http.ResponseWriter, r *http.Request) {
	...
	if !imaging.IsSafePathSegment(channelID) {
		handlers.RespondError(w, r, http.StatusBadRequest, "VALIDATION_ERROR", "invalid channel id")
		return
	}
	if h.imageDir == "" {
	...
	if err := os.WriteFile(dest, imgData, 0o644); err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "STORAGE_ERROR", "could not save logo")
		return
	}
...
func serveLocalChannelLogo(w http.ResponseWriter, r *http.Request, imageDir string, logger *slog.Logger, channelID, basename string) {
	if imageDir == "" {
		handlers.RespondError(w, r, http.StatusNotFound, "NO_LOGO", "image storage not configured")
		return
	}
	if !imaging.IsSafePathSegment(basename) {
		...
		handlers.RespondError(w, r, http.StatusNotFound, "LOGO_UNAVAILABLE", "")
		return
	}
	...
	if err != nil {
		if errors.Is(err, os.ErrNotExist) {
			handlers.RespondError(w, r, http.StatusNotFound, "LOGO_UNAVAILABLE", "uploaded logo file is missing")
			return
		}
		logger.Error("read channel-logo file", "channel", channelID, "path", path, "error", err)
		handlers.RespondError(w, r, http.StatusInternalServerError, "LOGO_READ_FAILED", "")
		return
	}
...
func (h *iptvLogoHandler) RefreshLogosFromIPTVOrg(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		if strings.Contains(err.Error(), "not configured") {
			handlers.RespondError(w, r, http.StatusServiceUnavailable, "NO_IPTV_ORG", err.Error())
			return
		}
		h.logger.Error("iptv-org refresh failed", "library", libraryID, "error", err)
		handlers.RespondError(w, r, http.StatusBadGateway, "IPTV_ORG_ERROR", "could not refresh logos from iptv-org")
		return
	}
	handlers.RespondJSON(w, http.StatusOK, map[string]any{
		"data": map[string]any{
