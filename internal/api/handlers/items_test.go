...
func newItemTestEnv(t *testing.T) *itemTestEnv {
	...
	env.handler = NewItemHandler(env.svc, env.images, env.meta, env.userData, nil, env.chapters, nil, env.extIDs, env.people, nil, nil, nil, "", testutil.NopLogger())
...
func trickplayEnv(t *testing.T) (*itemTestEnv, string) {
	...
	dir := t.TempDir()
	env := newItemTestEnv(t)
	env.handler = NewItemHandler(env.svc, env.images, env.meta, env.userData, nil, env.chapters, nil, env.extIDs, env.people, nil, nil, nil, dir, testutil.NopLogger())
	r := chi.NewRouter()
