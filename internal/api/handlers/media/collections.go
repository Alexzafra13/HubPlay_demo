...
func NewCollectionHandler(collections CollectionRepository, overrides CollectionImageOverrideRepo, images CollectionImageProvider, imageDir string, audit handlers.AuditEmitter, logger *slog.Logger) *CollectionHandler {
...
func (h *CollectionHandler) auditEmit() handlers.AuditEmitter {
	if h.audit != nil {
		return h.audit
	}
...
func (h *CollectionHandler) List(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "list collections failed")
		return
	}
...
func (h *CollectionHandler) Get(w http.ResponseWriter, r *http.Request) {
	...
	rawID := handlers.RequireParam(w, r, "id")
	if rawID == "" {
		return
	}
	id := rawID
	...
	if err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "lookup failed")
		return
	}
	if col == nil {
		handlers.RespondError(w, r, http.StatusNotFound, "NOT_FOUND", "collection not found")
		return
	}
	...
	if err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "list items failed")
		return
	}
...
func (h *CollectionHandler) SetCollectionImage(w http.ResponseWriter, r *http.Request) {
	...
	if h.overrides == nil {
		handlers.RespondError(w, r, http.StatusServiceUnavailable, "NO_OVERRIDES", "collection image overrides not configured")
		return
	}
	...
	if req.URL == "" {
		handlers.RespondError(w, r, http.StatusBadRequest, "MISSING_URL", "url required (use DELETE to clear)")
		return
	}
	if !isHTTPURL(req.URL) {
		handlers.RespondError(w, r, http.StatusBadRequest, "INVALID_URL", "url must be http(s)")
		return
	}
	...
	if err := h.overrides.UpsertURL(r.Context(), collectionID, imageType, req.URL); err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "STORAGE_ERROR", "could not save override")
		return
	}
...
func (h *CollectionHandler) UploadCollectionImage(w http.ResponseWriter, r *http.Request) {
	...
	if h.imageDir == "" {
		handlers.RespondError(w, r, http.StatusServiceUnavailable, "NO_STORAGE", "image storage not configured")
		return
	}

	if err := r.ParseMultipartForm(imaging.MaxUploadBytes); err != nil {
		handlers.RespondError(w, r, http.StatusBadRequest, "VALIDATION_ERROR", "file too large (max 10MB)")
		return
	}
	file, _, err := r.FormFile("file")
	if err != nil {
		handlers.RespondError(w, r, http.StatusBadRequest, "VALIDATION_ERROR", "file field is required")
		return
	}
	...
	if err != nil {
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "failed to read file")
		return
	}
	...
	if !imaging.IsValidContentType(sniffed) {
		handlers.RespondError(w, r, http.StatusBadRequest, "VALIDATION_ERROR", "invalid file type (must be JPEG, PNG, or WebP)")
		return
	}
	if err := imaging.EnforceMaxPixels(imgData); err != nil {
		handlers.RespondError(w, r, http.StatusBadRequest, "VALIDATION_ERROR", "image dimensions too large")
		return
	}
	...
	if err := os.MkdirAll(dir, 0o755); err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "STORAGE_ERROR", "could not create storage dir")
		return
	}
	...
	if err := os.WriteFile(dest, imgData, 0o644); err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "STORAGE_ERROR", "could not save image")
		return
	}
...
func (h *CollectionHandler) ClearCollectionImage(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.overrides.Delete(r.Context(), collectionID, imageType); err != nil {
		handlers.RespondError(w, r, http.StatusInternalServerError, "STORAGE_ERROR", "could not clear override")
		return
	}
	if prev != nil && prev.File != "" {
		h.deleteCollectionImageFile(prev.File)
...
func (h *CollectionHandler) ServeCollectionImage(w http.ResponseWriter, r *http.Request) {
	...
	if h.overrides == nil || h.imageDir == "" {
		handlers.RespondError(w, r, http.StatusNotFound, "NO_OVERRIDE", "")
		return
	}
	ov, err := h.overrides.Get(r.Context(), collectionID, imageType)
	if err != nil || ov == nil || ov.File == "" {
		handlers.RespondError(w, r, http.StatusNotFound, "NO_OVERRIDE", "no file override for this collection image")
		return
	}
	...
	if err != nil {
		if errors.Is(err, os.ErrNotExist) {
			handlers.RespondError(w, r, http.StatusNotFound, "FILE_MISSING", "uploaded file is missing on disk")
			return
		}
		handlers.RespondError(w, r, http.StatusInternalServerError, "READ_FAILED", "")
		return
	}
	defer f.Close() //nolint:errcheck
	info, err := f.Stat()
	if err != nil {
		handlers.RespondError(w, r, http.StatusInternalServerError, "STAT_FAILED", "")
		return
	}
	var head [512]byte
	n, _ := f.Read(head[:])
	...
	if _, err := f.Seek(0, io.SeekStart); err != nil {
		handlers.RespondError(w, r, http.StatusInternalServerError, "SEEK_FAILED", "")
		return
	}
...
func (h *CollectionHandler) parseCollectionImageRoute(w http.ResponseWriter, r *http.Request) (string, string, bool) {
	...
	imageType := handlers.RequireParam(w, r, "type")
	if imageType == "" {
		return "", "", false
	}
	if imageType != "poster" && imageType != "backdrop" {
		handlers.RespondError(w, r, http.StatusBadRequest, "INVALID_TYPE", "image type must be poster or backdrop")
		return "", "", false
	}
...
func (h *CollectionHandler) AvailableCollectionImages(w http.ResponseWriter, r *http.Request) {
	...
	if h.images == nil {
		handlers.RespondError(w, r, http.StatusServiceUnavailable, "NO_PROVIDER", "image provider not configured")
		return
	}
	...
	if col.TMDBID == 0 {
		...
		handlers.RespondData(w, http.StatusOK, []any{})
		return
	}
	...
	if err != nil {
		...
		handlers.RespondError(w, r, http.StatusBadGateway, "PROVIDER_ERROR", "could not list images from provider")
		return
	}
