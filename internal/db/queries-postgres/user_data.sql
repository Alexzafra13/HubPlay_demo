...
DELETE FROM user_data WHERE user_id = $1 AND item_id = $2;
...
WHERE ud.user_id = $1 AND ud.completed = 0 AND ud.position_ticks > 0
  ...
  AND (
    ud.last_played_at >= $2  -- abandoned threshold (sqlc auto-names it LastPlayedAt)
...
WHERE s.parent_id = $2 AND s.type = 'season';
