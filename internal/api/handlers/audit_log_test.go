...
func mountAudit(h *handlers.AuditLogHandler) http.Handler {
	r := chi.NewRouter()
	r.Get("/audit", h.Query)
...
func TestAuditLogHandler_Query_HappyPath(t *testing.T) {
	...
	h := handlers.NewAuditLogHandler(store, slog.Default())
...
func TestAuditLogHandler_Query_RejectsMalformedFrom(t *testing.T) {
	h := handlers.NewAuditLogHandler(&fakeAuditStore{}, slog.Default())
