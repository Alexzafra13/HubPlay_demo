...
type TransmuxManagerConfig struct {
	...
	// Clock abstrae time.Now() para que los tests puedan controlar el
	// TTL del decodeMode fallback y el idle reaper sin depender del
	// reloj real. Nil = clock.New() (real). Cierra olor F15-2 para
	// este servicio.
	Clock clock.Clock
}
...
// se lean cortas en el resto del fichero.
func (m *TransmuxManager) now() time.Time {
...
func (m *TransmuxManager) promoteToReencode(channelID string) {
	...
	m.decodeMode[channelID] = decodeModeEntry{
		...
		expiresAt: m.now().Add(decodeModeFallbackTTL),
	}
...
func (m *TransmuxManager) GetOrStart(ctx context.Context, channelID, upstreamURL string) (*TransmuxSession, error) {
	...
	if s, ok := m.sessions[channelID]; ok && !s.stopped.Load() {
		...
		s.lastTouchUnixNano.Store(m.now().UnixNano())
		m.mu.Unlock()
