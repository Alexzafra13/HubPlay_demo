...
func (h *UploadBrowseHandler) Browse(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		handlers.RespondError(w, r, http.StatusBadRequest, "INVALID_PATH", err.Error())
		return
	}
	...
	if err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "READ_DIR_FAILED", err.Error())
		return
	}
	...
	w.Header().Set("Cache-Control", handlers.CacheControlListingShort)
	handlers.RespondJSON(w, http.StatusOK, map[string]any{
		"data": map[string]any{
			"library_id":   lib.ID,
			"library_name": lib.Name,
...
func (h *UploadBrowseHandler) CreateFolder(w http.ResponseWriter, r *http.Request) {
	...
	if canonical == "" {
		...
		// click no hace nada.
		handlers.RespondError(w, r, http.StatusBadRequest, "EMPTY_PATH",
	...
	if err := os.MkdirAll(abs, 0o755); err != nil {
		handlers.RespondError(w, r, http.StatusInternalServerError, "MKDIR_FAILED", err.Error())
		return
	}
...
func (h *UploadBrowseHandler) DeleteEntry(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "STAT_FAILED", err.Error())
		return
	}
	...
	if info.IsDir() && !recursive {
		...
		if len(entries) > 0 {
			handlers.RespondError(w, r, http.StatusConflict, "DIR_NOT_EMPTY",
...
func (h *UploadBrowseHandler) RenameEntry(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil || fromCanon == "" {
		handlers.RespondError(w, r, http.StatusBadRequest, "INVALID_FROM",
			"from path is invalid or empty")
		return
	}
	toCanon, err := upload.SanitizeSubpath(req.To)
	if err != nil || toCanon == "" {
		handlers.RespondError(w, r, http.StatusBadRequest, "INVALID_TO",
			"to path is invalid or empty")
		return
	}
	if fromCanon == toCanon {
		handlers.RespondError(w, r, http.StatusBadRequest, "SAME_PATH",
	...
	if err != nil {
		handlers.RespondError(w, r, http.StatusBadRequest, "INVALID_FROM", err.Error())
		return
	}
	toAbs, err := upload.ResolveSubpath(lib.Paths[0], toCanon)
	if err != nil {
		handlers.RespondError(w, r, http.StatusBadRequest, "INVALID_TO", err.Error())
		return
	}

	if _, err := os.Stat(fromAbs); err != nil {
		if os.IsNotExist(err) {
			handlers.RespondError(w, r, http.StatusNotFound, "FROM_NOT_FOUND",
	...
	if _, err := os.Stat(toAbs); err == nil {
		handlers.RespondError(w, r, http.StatusConflict, "TO_EXISTS",
	...
	if err := os.Rename(fromAbs, toAbs); err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "RENAME_FAILED", err.Error())
		return
	}
...
func (h *UploadBrowseHandler) resolveLibrary(w http.ResponseWriter, r *http.Request) (*librarymodel.Library, bool) {
	...
	if err != nil {
		handlers.RespondError(w, r, http.StatusInternalServerError, "LIST_FAILED", err.Error())
		return nil, false
	}
	for _, l := range libs {
		if l.ID == id {
			if len(l.Paths) == 0 {
				handlers.RespondError(w, r, http.StatusConflict, "LIBRARY_NO_PATHS",
	...
	handlers.RespondError(w, r, http.StatusNotFound, "NOT_FOUND", "library not found or not accessible")
	return nil, false
}
