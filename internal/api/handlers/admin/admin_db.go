...
func (h *AdminDBHandler) auditEmit() handlers.AuditEmitter {
	if h.audit != nil {
		return h.audit
	}
...
func (h *AdminDBHandler) Profiles(w http.ResponseWriter, r *http.Request) {
	handlers.RespondData(w, http.StatusOK, DetectDBProfiles())
}
...
func (h *AdminDBHandler) Save(w http.ResponseWriter, r *http.Request) {
	...
	if driver == db.DriverSQLite && strings.TrimSpace(req.Path) == "" {
		handlers.RespondError(w, r, http.StatusBadRequest, "VALIDATION_ERROR", "path is required for sqlite")
		return
	}
	dsn := req.DSN
	if driver == db.DriverPostgres && req.UseBundled {
		...
		if dsn == "" {
			handlers.RespondError(w, r, http.StatusBadRequest, "VALIDATION_ERROR", "no bundled Postgres available — paste a custom DSN instead")
			return
		}
	}
	if driver == db.DriverPostgres && strings.TrimSpace(dsn) == "" {
		handlers.RespondError(w, r, http.StatusBadRequest, "VALIDATION_ERROR", "dsn is required for postgres")
		return
	}
...
func (h *AdminDBHandler) Restart(w http.ResponseWriter, r *http.Request) {
	if h.restart == nil {
		handlers.RespondError(w, r, http.StatusServiceUnavailable, "NOT_AVAILABLE", "restart is not wired in this build")
		return
	}
...
func (h *AdminDBHandler) Migrate(w http.ResponseWriter, r *http.Request) {
	if h.cfg.Database.Driver != db.DriverSQLite {
		handlers.RespondError(w, r, http.StatusBadRequest, "MIGRATE_WRONG_DRIVER",
	...
	if strings.TrimSpace(req.TargetDSN) == "" {
		handlers.RespondError(w, r, http.StatusBadRequest, "VALIDATION_ERROR", "target_dsn is required")
		return
	}

	flusher, ok := w.(http.Flusher)
	if !ok {
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "response writer does not support flushing")
		return
	}
	w.Header().Set("Content-Type", "application/x-ndjson")
	w.Header().Set("Cache-Control", handlers.CacheControlNoStore)
