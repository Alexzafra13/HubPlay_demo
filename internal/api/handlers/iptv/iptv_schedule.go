...
// with IPTVHandler via the same handlers.LibraryAccessService / RequireAdmin
...
func (h *IPTVScheduleHandler) Upsert(w http.ResponseWriter, r *http.Request) {
	...
	kind := handlers.RequireParam(w, r, "kind")
	if kind == "" {
		return
	}
	if !h.canAccess(r, libraryID) {
	...
	if !isValidJobKind(kind) {
		handlers.RespondError(w, r, http.StatusBadRequest, "INVALID_KIND", "unknown job kind")
		return
	}
	...
	if body.IntervalHours < 1 || body.IntervalHours > 720 {
		handlers.RespondError(w, r, http.StatusBadRequest, "INVALID_INTERVAL",
	...
	handlers.RespondData(w, http.StatusOK, jobToDTO(saved))
}
...
func (h *IPTVScheduleHandler) RunNow(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.runner.RunNow(r.Context(), libraryID, kind); err != nil {
		h.logger.Warn("iptv schedule run-now failed",
			"library", libraryID, "kind", kind, "error", err)
		handlers.RespondError(w, r, http.StatusBadGateway, "REFRESH_FAILED", err.Error())
		return
	}
