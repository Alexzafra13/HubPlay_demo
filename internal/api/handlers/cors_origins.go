...
func (h *CorsOriginsHandler) List(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		respondError(w, r, http.StatusInternalServerError, "CORS_LIST_FAILED", err.Error())
		return
	}
...
func (h *CorsOriginsHandler) Add(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		respondError(w, r, http.StatusBadRequest, "INVALID_ORIGIN", err.Error())
		return
	}
	...
	for _, s := range h.registry.Statics() {
		if s == canonical {
			respondError(w, r, http.StatusConflict, "ALREADY_STATIC",
	...
	if err := h.store.Insert(r.Context(), row); err != nil {
		...
		respondError(w, r, http.StatusInternalServerError, "INSERT_FAILED", err.Error())
		return
	}
...
func (h *CorsOriginsHandler) Delete(w http.ResponseWriter, r *http.Request) {
	...
	if raw == "" {
		respondError(w, r, http.StatusBadRequest, "MISSING_ORIGIN",
	...
	if err != nil {
		respondError(w, r, http.StatusBadRequest, "BAD_ORIGIN", err.Error())
		return
	}
	...
	for _, s := range h.registry.Statics() {
		if s == decoded {
			respondError(w, r, http.StatusConflict, "IMMUTABLE_STATIC",
