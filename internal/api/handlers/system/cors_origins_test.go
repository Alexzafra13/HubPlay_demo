...
func mountCors(h *system.CorsOriginsHandler) http.Handler {
	r := chi.NewRouter()
	r.Get("/cors", h.List)
	r.Post("/cors", h.Add)
...
func TestCorsOriginsHandler_List_EmptyDynamics(t *testing.T) {
	...
	h := system.NewCorsOriginsHandler(store, reg, okValidator, nil, slog.Default())
...
func TestCorsOriginsHandler_Add_RejectsBadValidator(t *testing.T) {
	store := &fakeCorsStore{}
	reg := &fakeRegistry{}
	h := system.NewCorsOriginsHandler(store, reg, rejectValidator, nil, slog.Default())
