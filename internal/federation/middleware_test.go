...
func (r *inMemoryFedRepo) ListCachedItems(_ context.Context, peerID, libraryID string, offset, limit int) (CachedItemPage, error) {
	...
	if offset >= total {
		return CachedItemPage{Items: []*SharedItem{}, Total: total, LastSync: entry.cachedAt}, nil
	}
	end := offset + limit
	if end > total || limit <= 0 {
		end = total
	}
	return CachedItemPage{Items: entry.items[offset:end], Total: total, LastSync: entry.cachedAt}, nil
}
