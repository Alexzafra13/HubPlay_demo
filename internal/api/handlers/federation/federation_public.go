...
func (h *FederationPublicHandler) ServerInfo(w http.ResponseWriter, r *http.Request) {
	...
	handlers.RespondJSON(w, http.StatusOK, infoToWire(info))
}
...
func (h *FederationPublicHandler) ReceivePairingRequest(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		handlers.RespondError(w, r, http.StatusBadRequest, "INVALID_REQUESTER", "requester info malformed")
		return
	}
	...
	if err != nil {
		if errors.Is(err, domain.ErrPairingRequestsDisabled) {
			...
			handlers.RespondError(w, r, http.StatusForbidden, "PAIRING_REQUESTS_DISABLED",
				"this server is not accepting pairing requests")
			return
		}
		if errors.Is(err, domain.ErrPairingRequestQuotaExceeded) {
			w.Header().Set("Retry-After", "300")
			handlers.RespondError(w, r, http.StatusTooManyRequests, "QUOTA_EXCEEDED",
				"too many pending requests; retry later")
			return
		}
		if errors.Is(err, domain.ErrAlreadyExists) {
			handlers.RespondError(w, r, http.StatusConflict, "PEER_ALREADY_PAIRED", "this peer is already paired")
			return
		}
		var ve *domain.ValidationError
		if errors.As(err, &ve) {
			handlers.RespondError(w, r, http.StatusBadRequest, "INVALID_REQUEST", "request body validation failed")
			return
		}
		h.logger.Warn("federation: receive pairing request failed", "error", err)
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "could not accept the request")
		return
	}
	handlers.RespondJSON(w, http.StatusAccepted, pairingRequestResponse{
		ID:        pending.ID,
...
func (h *FederationPublicHandler) ReceivePairingCallback(w http.ResponseWriter, r *http.Request) {
	requestID := handlers.RequireParam(w, r, "id")
	if requestID == "" {
		return
	}
	...
	if err != nil {
		handlers.RespondError(w, r, http.StatusBadRequest, "INVALID_ACCEPTER", "accepter info malformed")
		return
	}
	sig, err := federation.DecodePublicKey(body.Signature)
	if err != nil {
		handlers.RespondError(w, r, http.StatusBadRequest, "INVALID_SIGNATURE", "signature base64 invalid")
		return
	}
	if err := h.mgr.HandlePairingCallback(r.Context(), requestID, body.Outcome, body.RequestToken, accepter, sig); err != nil {
		...
		handlers.RespondError(w, r, http.StatusBadRequest, "CALLBACK_REJECTED", "callback validation failed")
		return
	}
...
func (h *FederationPublicHandler) ReceivePairingCancel(w http.ResponseWriter, r *http.Request) {
	...
	if pending.Direction != federation.PendingDirectionIncoming {
		handlers.RespondError(w, r, http.StatusBadRequest, "WRONG_DIRECTION", "only incoming requests can be cancelled")
		return
	}
	if pending.RequestToken != body.RequestToken {
		handlers.RespondError(w, r, http.StatusForbidden, "TOKEN_MISMATCH", "request token mismatch")
		return
	}
	...
	if err := h.mgr.CancelIncomingPairingRequest(r.Context(), requestID); err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "could not cancel")
		return
	}
	w.WriteHeader(http.StatusNoContent)
}
...
func (h *FederationPublicHandler) ServeIdentityAvatar(w http.ResponseWriter, r *http.Request) {
	...
	if relName == "" {
		handlers.RespondError(w, r, http.StatusNotFound, "NOT_FOUND", "server has no avatar")
		return
	}
	full, err := h.mgr.IdentityAvatarFilePath(relName)
	if err != nil {
		handlers.RespondError(w, r, http.StatusInternalServerError, "AVATAR_PATH", err.Error())
		return
	}
	f, err := os.Open(full)
	if err != nil {
		if os.IsNotExist(err) {
			handlers.RespondError(w, r, http.StatusNotFound, "NOT_FOUND", "avatar file missing")
			return
		}
		handlers.RespondError(w, r, http.StatusInternalServerError, "AVATAR_READ", err.Error())
		return
	}
	...
	stat, err := f.Stat()
	if err != nil {
		handlers.RespondError(w, r, http.StatusInternalServerError, "AVATAR_STAT", err.Error())
		return
	}
	...
	w.Header().Set("Cache-Control", handlers.CacheControlMediumPublic)
...
func (h *FederationPublicHandler) ListLibraries(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "failed to list libraries")
		return
	}
	if libs == nil {
		libs = []*federation.SharedLibrary{}
	}
	handlers.RespondJSON(w, http.StatusOK, libs)
}
...
func (h *FederationPublicHandler) ListLibraryItems(w http.ResponseWriter, r *http.Request) {
	...
	libraryID := handlers.RequireParam(w, r, "libraryID")
	if libraryID == "" {
		return
	}
	...
	if err != nil {
		if errors.Is(err, domain.ErrPeerNotFound) {
			handlers.RespondError(w, r, http.StatusNotFound, "LIBRARY_NOT_FOUND", "library not found")
			return
		}
		h.logger.Error("federation: list shared items", "error", err)
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "failed to list items")
		return
	}
...
func (h *FederationPublicHandler) SearchLibraries(w http.ResponseWriter, r *http.Request) {
	...
	if query == "" {
		handlers.RespondError(w, r, http.StatusBadRequest, "INVALID_REQUEST", "q required")
		return
	}
	...
	if err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "search failed")
		return
	}
	if items == nil {
...
func (h *FederationPublicHandler) ListRecent(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "list recent failed")
		return
	}
	if items == nil {
		items = []*federation.SharedItem{}
...
func (h *FederationPublicHandler) Handshake(w http.ResponseWriter, r *http.Request) {
	...
	if req.Code == "" {
		handlers.RespondError(w, r, http.StatusBadRequest, "FEDERATION_CODE_REQUIRED", "code required")
		return
	}
	remote, err := wireToInfo(req.RemoteInfo)
	if err != nil {
		handlers.RespondError(w, r, http.StatusBadRequest, "FEDERATION_REMOTE_INFO_INVALID", "invalid remote_info: "+err.Error())
		return
	}
	...
	if err != nil {
		...
		handlers.RespondError(w, r, status, code, userMsg)
		return
	}
	// Bare ServerInfo (no envelope) for peer-to-peer compatibility.
	handlers.RespondJSON(w, http.StatusOK, infoToWire(ours))
}
