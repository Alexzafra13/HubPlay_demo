...
func NewProgressHandler(userData handlers.UserDataRepository, images handlers.ImageRepository, bus handlers.EventBusPublisher, logger *slog.Logger) *ProgressHandler {
...
func (h *ProgressHandler) GetProgress(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "failed to get progress")
		return
	}
...
func (h *ProgressHandler) UpdateProgress(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.userData.UpdateProgress(r.Context(), claims.UserID, itemID, req.PositionTicks, completed); err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "failed to update progress")
		return
	}
...
func (h *ProgressHandler) MarkPlayed(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.userData.MarkPlayed(r.Context(), claims.UserID, itemID); err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "failed to mark played")
		return
	}
...
func (h *ProgressHandler) MarkUnplayed(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.userData.Delete(r.Context(), claims.UserID, itemID); err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "failed to mark unplayed")
		return
	}
	h.publish(event.PlayedToggled, claims.UserID, itemID, map[string]any{
		"played":    false,
...
func (h *ProgressHandler) ToggleFavorite(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "failed to get favorite state")
		return
	}
	...
	if err := h.userData.SetFavorite(r.Context(), claims.UserID, itemID, newState); err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "failed to toggle favorite")
		return
	}
...
func (h *ProgressHandler) RemoveFromContinueWatching(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.userData.ClearProgress(r.Context(), claims.UserID, itemID); err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "failed to remove from continue watching")
		return
	}
...
func (h *ProgressHandler) ContinueWatching(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "failed to get continue watching")
		return
	}
	...
	for _, item := range items {
		...
		if urls, ok := imageMap[item.ItemID]; ok {
			if u, ok := urls["primary"]; ok {
				...
				handlers.AttachPosterPlaceholder(entry, u)
			}
...
func (h *ProgressHandler) Favorites(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "failed to get favorites")
		return
	}
...
func (h *ProgressHandler) NextUp(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "failed to get next up")
		return
	}
