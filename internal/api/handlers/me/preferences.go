...
func (h *PreferencesHandler) SetMine(w http.ResponseWriter, r *http.Request) {
	...
	if len(key) > 128 {
		handlers.RespondError(w, r, http.StatusBadRequest, "INVALID_KEY", "key must be 1-128 chars")
		return
	}
	...
	if len(body.Value) > 8*1024 {
		handlers.RespondError(w, r, http.StatusBadRequest, "VALUE_TOO_LARGE", "value must be ≤ 8 KB")
		return
	}
	...
	if !utf8.ValidString(body.Value) {
		handlers.RespondError(w, r, http.StatusBadRequest, "INVALID_UTF8", "value must be valid UTF-8")
		return
	}
	if strings.ContainsRune(body.Value, 0) {
		handlers.RespondError(w, r, http.StatusBadRequest, "INVALID_VALUE", "value must not contain null bytes")
		return
	}
