...
// HomeHandler. 1 de 14 métodos de handlers.UserService.
...
func NewLibraryHandler(lib libraryOps, images handlers.ImageRepository, metadata handlers.MetadataRepository, userData handlers.UserDataRepository, users userProfileLookup, audit handlers.AuditEmitter, logger *slog.Logger) *LibraryHandler {
	return &LibraryHandler{lib: lib, images: images, metadata: metadata, userData: userData, users: users, audit: audit, logger: logger}
}

func (h *LibraryHandler) auditEmit() handlers.AuditEmitter {
	if h.audit != nil {
		return h.audit
	}
...
func (h *LibraryHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	handlers.RespondData(w, http.StatusCreated, libraryResponse(lib))
}
...
func (h *LibraryHandler) List(w http.ResponseWriter, r *http.Request) {
	...
	handlers.RespondData(w, http.StatusOK, items)
}
...
func (h *LibraryHandler) Update(w http.ResponseWriter, r *http.Request) {
	...
	handlers.RespondData(w, http.StatusOK, libraryResponse(lib))
}
...
func (h *LibraryHandler) Browse(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		handlers.RespondError(w, r, http.StatusBadRequest, "BROWSE_ERROR", "invalid path")
		return
	}

	// Block access to sensitive system directories
	if isSensitiveBrowsePath(absPath) {
		handlers.RespondError(w, r, http.StatusForbidden, "BROWSE_ERROR", "access denied: cannot browse system directories")
		return
	}

	entries, err := os.ReadDir(absPath)
	if err != nil {
		handlers.RespondError(w, r, http.StatusBadRequest, "BROWSE_ERROR", "directory not found or not accessible")
		return
	}
	...
	w.Header().Set("Cache-Control", handlers.CacheControlListing)
...
func (h *LibraryHandler) AllItems(w http.ResponseWriter, r *http.Request) {
	q := r.URL.Query()
	offset, limit, _ := handlers.ParsePaginationFromValues(w, r, q)
...
func (h *LibraryHandler) Genres(w http.ResponseWriter, r *http.Request) {
	...
	if itemType != "" && itemType != "movie" && itemType != "series" && itemType != "episode" {
		handlers.RespondError(w, r, http.StatusBadRequest, "VALIDATION_ERROR", "type must be one of movie, series, episode")
		return
	}
			...
			for i, item := range items {
				if urls, ok := imageURLs[item.ID]; ok {
					if poster, ok := urls["primary"]; ok {
						...
						handlers.AttachPosterPlaceholder(data[i], poster)
					}
			...
			if err != nil {
				...
				for i, item := range items {
					if ud, ok := userDataByID[item.ID]; ok {
						data[i]["user_data"] = handlers.UserDataResponse(ud, item.DurationTicks)
					}
				}
			}
		}
...
// itemSummaryResponse was moved to the root handlers package as
// handlers.ItemSummaryResponse. Callers in this package reference it
