...
func NewUserHandler(users handlers.UserService, libraries userLibraryOps, audit handlers.AuditEmitter, logger *slog.Logger) *UserHandler {
	return &UserHandler{
		users:     users,
		libraries: libraries,
		audit:     audit,
...
func (h *UserHandler) auditEmit() handlers.AuditEmitter {
	if h.audit != nil {
		return h.audit
	}
...
func (h *UserHandler) SetRole(w http.ResponseWriter, r *http.Request) {
	...
	if req.Role != "user" && req.Role != "admin" {
		handlers.RespondError(w, r, http.StatusBadRequest, "VALIDATION_ERROR", "role must be 'user' or 'admin'")
		return
	}
	if primaryID, _ := h.users.PrimaryAdminID(r.Context()); primaryID != "" && primaryID == id {
		handlers.RespondError(w, r, http.StatusForbidden, "PRIMARY_ADMIN_LOCKED",
...
func (h *UserHandler) SetAccess(w http.ResponseWriter, r *http.Request) {
	...
	if req.DurationDays < 0 {
		handlers.RespondError(w, r, http.StatusBadRequest, "VALIDATION_ERROR", "duration_days must be non-negative")
		return
	}
...
func (h *UserHandler) SetActive(w http.ResponseWriter, r *http.Request) {
	...
	if claims != nil && claims.UserID == id {
		handlers.RespondError(w, r, http.StatusBadRequest, "BAD_REQUEST", "cannot deactivate your own account")
		return
	}
	if primaryID, _ := h.users.PrimaryAdminID(r.Context()); primaryID != "" && primaryID == id {
		handlers.RespondError(w, r, http.StatusForbidden, "PRIMARY_ADMIN_LOCKED",
...
func (h *UserHandler) ServeUserAvatar(w http.ResponseWriter, r *http.Request) {
	...
	if target.AvatarPath == "" {
		handlers.RespondError(w, r, http.StatusNotFound, "NOT_FOUND", "user has no avatar")
		return
	}
...
func (h *UserHandler) Delete(w http.ResponseWriter, r *http.Request) {
	...
	claims := auth.GetClaims(r.Context())
	if claims != nil && claims.UserID == id {
		handlers.RespondError(w, r, http.StatusBadRequest, "BAD_REQUEST", "cannot delete your own account")
		return
	}
