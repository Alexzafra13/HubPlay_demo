...
func NewAdminLogsHandler(buffer *logging.Buffer, limiter *handlers.SSELimiter) *AdminLogsHandler {
...
func (h *AdminLogsHandler) Stream(w http.ResponseWriter, r *http.Request) {
	...
	if h.buffer == nil {
		handlers.RespondError(w, r, http.StatusServiceUnavailable, "LOGS_UNAVAILABLE",
			"log buffer is not configured for this build")
		return
	}
	flusher, ok := w.(http.Flusher)
	if !ok {
		handlers.RespondError(w, r, http.StatusInternalServerError, "STREAMING_UNSUPPORTED",
