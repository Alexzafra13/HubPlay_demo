...
func (h *PeopleHandler) Thumb(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if errors.Is(err, domain.ErrNotFound) {
			handlers.RespondError(w, r, http.StatusNotFound, "NOT_FOUND", "person not found")
			return
		}
	...
	if person.ThumbPath == "" {
		handlers.RespondError(w, r, http.StatusNotFound, "NOT_FOUND", "person has no thumb")
		return
	}
	if !h.isUnderImageDir(person.ThumbPath) {
		h.logger.Warn("person thumb_path escapes imageDir — refusing to serve", "id", id)
		handlers.RespondError(w, r, http.StatusNotFound, "NOT_FOUND", "person thumb missing")
		return
	}
...
func (h *PeopleHandler) Get(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "filmography lookup failed")
		return
	}
