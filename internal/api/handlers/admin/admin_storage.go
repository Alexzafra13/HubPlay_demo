...
func (h *AdminStorageHandler) Disks(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL", "failed to list libraries")
		return
	}
	sizes, err := h.items.SumItemSizesByLibrary(r.Context())
	if err != nil {
		h.logger.Error("sum item sizes", "error", err)
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL", "failed to compute library sizes")
		return
	}
