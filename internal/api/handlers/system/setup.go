...
type SetupHandler struct {
	...
	providers handlers.ProviderRepository
...
func (h *SetupHandler) requireSetupActive(w http.ResponseWriter, r *http.Request) bool {
	...
	handlers.RespondError(w, r, http.StatusForbidden, "SETUP_COMPLETE", "setup wizard is no longer accepting requests")
	return false
}
...
func (h *SetupHandler) Browse(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		handlers.RespondError(w, r, http.StatusBadRequest, "BROWSE_ERROR", "cannot browse this directory")
		return
	}
...
func (h *SetupHandler) CreateLibraries(w http.ResponseWriter, r *http.Request) {
	...
	if len(req.Libraries) == 0 {
		handlers.RespondError(w, r, http.StatusBadRequest, "VALIDATION_ERROR", "at least one library is required")
		return
	}
	...
	handlers.RespondData(w, http.StatusCreated, created)
}
...
func (h *SetupHandler) UpdateSettings(w http.ResponseWriter, r *http.Request) {
	...
	if req.TMDbAPIKey != "" && h.providers != nil {
		...
		if err := h.providers.Upsert(r.Context(), cfg); err != nil {
			...
			handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "failed to save TMDB API key")
			return
		}
...
func (h *SetupHandler) Capabilities(w http.ResponseWriter, r *http.Request) {
	...
	handlers.RespondData(w, http.StatusOK, caps)
}
...
func (h *SetupHandler) DatabaseProfiles(w http.ResponseWriter, r *http.Request) {
	...
	handlers.RespondData(w, http.StatusOK, admin.DetectDBProfiles())
}
...
func (h *SetupHandler) TestDatabase(w http.ResponseWriter, r *http.Request) {
	...
	resp := admin.TestCandidateDB(r.Context(), req, h.logger)
...
func (h *SetupHandler) SaveDatabase(w http.ResponseWriter, r *http.Request) {
	...
	if h.dbSaver == nil {
		handlers.RespondError(w, r, http.StatusServiceUnavailable, "NOT_AVAILABLE", "database saver not wired")
		return
	}
	...
	if req.Driver != db.DriverSQLite && req.Driver != db.DriverPostgres {
		handlers.RespondError(w, r, http.StatusBadRequest, "VALIDATION_ERROR", "driver must be 'sqlite' or 'postgres'")
		return
	}
	if err := h.dbSaver.SaveDatabaseConfig(req.Driver, req.Path, req.DSN); err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "failed to persist database config")
		return
	}
...
func (h *SetupHandler) Complete(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.setup.CompleteSetup(req.StartScan); err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "failed to complete setup")
		return
	}
