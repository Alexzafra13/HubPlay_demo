...
func (h *SettingsHandler) List(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL", "failed to read settings")
		return
	}
	handlers.RespondData(w, http.StatusOK, settingsResponse{Settings: descriptors})
}
...
func (h *SettingsHandler) Update(w http.ResponseWriter, r *http.Request) {
	...
	if !isAllowedSettingKey(body.Key) {
		handlers.RespondError(w, r, http.StatusBadRequest, "UNKNOWN_KEY",
	...
	if err != nil {
		handlers.RespondError(w, r, http.StatusBadRequest, "INVALID_VALUE", err.Error())
		return
	}

	if err := h.settings.Set(r.Context(), body.Key, normalised); err != nil {
		h.logger.Error("set setting", "key", body.Key, "error", err)
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL", "failed to persist setting")
		return
	}
	...
	if err != nil {
		...
		handlers.RespondData(w, http.StatusOK, map[string]any{"key": body.Key, "value": normalised})
		return
	}
	handlers.RespondData(w, http.StatusOK, settingsResponse{Settings: descriptors})
}
...
func (h *SettingsHandler) Reset(w http.ResponseWriter, r *http.Request) {
	key := handlers.RequireParam(w, r, "key")
	if key == "" {
		return
	}
	...
	if err := h.settings.Delete(r.Context(), key); err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL", "failed to clear setting")
		return
	}
	...
	descriptors, err := h.describeAll(r.Context())
	if err != nil {
		handlers.RespondData(w, http.StatusOK, map[string]any{"key": key, "reset": true})
		return
	}
	handlers.RespondData(w, http.StatusOK, settingsResponse{Settings: descriptors})
}
