...
func mountBrowse(h *uploads.UploadBrowseHandler) http.Handler {
	r := chi.NewRouter()
...
func TestUploadBrowse_ListsSubdirs(t *testing.T) {
	...
	h := uploads.NewUploadBrowseHandler(svc, testutil.NopLogger())
...
func mountBrowseFull(h *uploads.UploadBrowseHandler) http.Handler {
	r := chi.NewRouter()
	r.Get("/libraries/{id}/upload-browse", h.Browse)
	r.Post("/libraries/{id}/folders", h.CreateFolder)
