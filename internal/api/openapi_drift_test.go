...
var outOfScopeExact = map[string]string{
	...
	"GET /torrent/play":                      "play-mode decision (direct vs HLS remux), web-only opt-in",
	"GET /torrent/hls/{infohash}/index.m3u8": "HLS remux playlist of an active torrent transcode, web-only opt-in",
	"GET /torrent/hls/{infohash}/{segment}":  "HLS remux segment of an active torrent transcode, web-only opt-in",
	...
	"DELETE /torrent/downloads/{id}":         "dismiss a terminal download job, web-only opt-in",
