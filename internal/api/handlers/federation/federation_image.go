...
type FederationImageHandler struct {
	...
	imageSrv imageServer // for ServeImageByID — same path-mapping + thumb cache
	logger   *slog.Logger
}
...
func (h *FederationImageHandler) ItemPoster(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		handlers.RespondError(w, r, http.StatusInternalServerError, "INTERNAL_ERROR", "image lookup failed")
		return
	}
	primary, ok := urls[itemID]["primary"]
	if !ok || primary.Path == "" {
		handlers.RespondError(w, r, http.StatusNotFound, "POSTER_NOT_FOUND", "no poster for item")
		return
	}
